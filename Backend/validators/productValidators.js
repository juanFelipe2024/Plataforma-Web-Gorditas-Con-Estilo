...
const reglasBase = [
  ...
  body("categoria")
    ...
    .withMessage(
      `La categoría debe ser una de: ${CATEGORIAS_VALIDAS.join(", ")}`,
    ),

  body("tallas")
    .optional()
    .isArray()
    .withMessage("Las tallas deben enviarse como arreglo")
    .custom((tallas) => {
      for (const item of tallas) {
        if (!item.talla || !TALLAS_VALIDAS.includes(item.talla)) {
          throw new Error(
            `Talla no válida: ${item.talla}. Debe ser una de: ${TALLAS_VALIDAS.join(", ")}`,
          );
        }
        if (
          ...
          throw new Error(
            `Stock inválido para la talla ${item.talla}. Debe ser un número entero mayor o igual a 0`,
          );
        }
      }
      return true;
  ...
  body("color")
    ...
    .trim()
    .isLength({ max: 50 })
    .withMessage("El color no puede superar 50 caracteres"),
